...
string AGXB.captchaImage({ "width":int, "height":int, "length":int, "type":string }) // 生成验证码图片, 返回图片srcURL字符串, 尺寸参数必传, 单位为逻辑点, 验证码默认长度4, 默认类型数字加字母
bool AGXB.verifyCaptcha("input captcha code") // 校验验证码正确与否
